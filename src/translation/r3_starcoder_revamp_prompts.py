...
def main(args):
    ...
    out_folder = f'output/StarCoder/{args.dataset}/{args.source_lang}/{args.target_lang}'
    ...
    model = AutoModelForCausalLM.from_pretrained('bigcode/starcoder', use_auth_token=os.environ['STARCODER_AUTH_TOKEN'], cache_dir='./huggingface', device_map="auto", load_in_8bit=True,max_memory=get_gpus_max_memory("45GB"))
    ...
    for f in tqdm(in_files):
        ...
        with open(prompt_file, "r", encoding="ISO-8859-1", errors='ignore') as fin:
            ...
            try:
                ...
                data = result[result.find('<fim_suffix><fim_middle>')+len('<fim_suffix><fim_middle>'):]
...
if __name__ == "__main__":
    ...
    logging.info(f"translating examples from {source} to {target} using StarCoder and {args.dataset} dataset")
    main(args)
