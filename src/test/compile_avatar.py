...
def main(args):
    ...
    dataset = 'avatar'
    translation_dir = f"translation_output/{args.model}/{dataset}/{args.source_lang}/{args.target_lang}"
    test_dir = f"dataset/{dataset}/{args.source_lang}/TestCases"
    os.makedirs(args.report_dir, exist_ok=True)
    files = [f for f in os.listdir(translation_dir) if f.split(".")[-1] in ["py", "java", "c", "c++", "go"]]
